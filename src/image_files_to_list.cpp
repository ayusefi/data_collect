...
ImageFilesToList::ImageFilesToList(ros::NodeHandle node, ros::NodeHandle private_nh)
{
  private_nh.param<std::string>("directory_path", _directory_path, "");
  ROS_INFO("directory_path: %s", _directory_path.c_str());
}

void ImageFilesToList::imageFilesToList()
{
  ...
  while (it != boost::filesystem::directory_iterator{}){
    ...
    timestamp_str.erase(remove(timestamp_str.begin(), timestamp_str.end(), '.'), timestamp_str.end());
        std::string line;
        MyFile << std::setprecision(10) << timestamp_str << ',' << (*it).path().stem().string() << ".jpg" << '\n';
    *it++;
  }
  MyFile.close();
